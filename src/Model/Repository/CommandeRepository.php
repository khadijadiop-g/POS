...
private Database $dtb;
 ...
 public  function __construct(Database $dtb)
    ...
    public static function getLignesByCommande(int $commandeId): array
    {
        $sql = "SELECT lc.*, p.libelle
                FROM lignes_commande lc
                JOIN produits p ON p.id = lc.produit_id
                WHERE lc.commande_id = :commande_id";
        return CommandeRepository::$dtb->executeQuery($sql, ['commande_id' => $commandeId], false);
    }

    public static function saveCommande(int $clientId, int $utilisateurId, ?int $modePaiementId, float $montantTotal, float $montantVerse): int
    {
        ...
        return CommandeRepository::$dtb->executeUpdate($sql, [
            'client_id' => $clientId,
    ...
    public static function saveLigneCommande(int $commandeId, int $produitId, int $qteCommande, float $prixReel): int
    {
